...
namespace BarTasca.Tests.Unit.Services.Tickets
{
    public class SkipTest
    {
        ...
        public async Task TicketNotFound_ReturnsNull()
        {
            ...
            var result = await service.SkipAsync(ticketId);
        ...
        // If valid transition from active ticket, updates status to Skipped and saves changes
        [Fact]
        public async Task ValidTransition_UpdatesStatusAndSaves()
        {
            ...
            _mapper.Map<TicketDetailDto>(ticket)
                   ...
                   {
                       Id = ticket.Id,
                       Status = TicketStatus.Skipped.ToString()
                   });

            _notifications.BroadcastTicketUpdatedAsync(Arg.Any<Ticket>(), Arg.Any<int>(), Arg.Any<CancellationToken>())
                          .Returns(Task.CompletedTask);

            var result = await svc.SkipAsync(ticketId);

            result.Should().NotBeNull();
            result!.Status.Should().Be(TicketStatus.Skipped.ToString());
        ...
        public async Task InvalidTransition_ThrowsException()
        {
            ...
            _tickets.GetByIdAsync(ticketId, Arg.Any<CancellationToken>())
                    .Returns(ticket);
            Func<Task> act = async () => await svc.SkipAsync(ticketId);
        ...
        // If ticket already Skipped, does not save changes but still broadcasts update
        [Fact]
